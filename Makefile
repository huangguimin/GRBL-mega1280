...
flash:	all
	$(AVRDUDE) -U flash:w:mcx314.hex:i
...
load: all
	bootloadHID mcx314.hex

clean:
	rm -f mcx314.hex $(BUILDDIR)/*.o $(BUILDDIR)/*.d $(BUILDDIR)/*.elf

# file targets:
$(BUILDDIR)/main.elf: $(OBJECTS)
	$(COMPILE) -o $(BUILDDIR)/main.elf $(OBJECTS) -lm -Wl,--gc-sections

mcx314.hex: $(BUILDDIR)/main.elf
	rm -f mcx314.hex
	avr-objcopy -j .text -j .data -O ihex $(BUILDDIR)/main.elf mcx314.hex
